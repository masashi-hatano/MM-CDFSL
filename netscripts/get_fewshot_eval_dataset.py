...
from datamodule.dataset.meccano_fewshot_eval_dataset import \
    MECCANOFewshotEvalDataset
...
def get_fewshot_eval_dataset(cfg, transform, mask_gen, num_frames, mode):
    if cfg.target_dataset.lower() == "epic":
        fewshot_eval_dataset = EPICFewshotEvalDataset(cfg, transform, mask_gen, num_frames, mode)
    elif cfg.target_dataset.lower() == "meccano":
        fewshot_eval_dataset = MECCANOFewshotEvalDataset(cfg, transform, mask_gen, num_frames, mode)
    elif cfg.target_dataset.lower() == "wear":
        fewshot_eval_dataset = WEARFewshotEvalDataset(cfg, transform, mask_gen, num_frames, mode)
    else:
