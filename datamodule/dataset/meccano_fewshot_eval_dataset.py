...
class MECCANOFewshotEvalDataset(torch.utils.data.Dataset):
    def __init__(self, cfg, transform, mask_gen, num_frames, mode):
        super(MECCANOFewshotEvalDataset, self).__init__()
        self.cfg = cfg
    ...
    def _construct_loader(self, cfg):
        ...
        for i, clip_dict in enumerate(data["clips"]):
            ...
            action_idx = clip_dict["action_id_internal"]
            action_label = clip_dict["action_name"]
        ...
        logger.info(f"Constructing MECCANO dataloader (size: {len(self._start_frame)})")

    def _get_frame(self, dir_to_img_frame, frame_name, mode, frames):
        if mode == "RGB":
            path = dir_to_img_frame / Path(f"{str(frame_name).zfill(5)}.jpg")
            if path.exists():
            ...
            path = Path(dir_to_flow_frame, f"{str(frame_name).zfill(5)}.npy")
            if path.exists():
