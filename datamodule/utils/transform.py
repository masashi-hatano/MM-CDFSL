...
class Stack(object):
    ...
    def stack_images(self, img_group):
    ...
    def __call__(self, img_tuple):
        img_group, label = img_tuple
        img = self.stack_images(img_group)
        return (img, label)
...
class ToTorchFormatTensor(object):
    ...
    def convert_to_tensor(self, pic):
    ...
    def __call__(self, pic_tuple):
        pic, label = pic_tuple
        img = self.convert_to_tensor(pic)
...
class GaussianNoise(object):
    ...
    def __call__(self, x):
        x_gn = x + self.scale * torch.randn_like(x)
        return x_gn
